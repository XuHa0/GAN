...
class Generator(keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = layers.Conv2DTranspose(256, 3, 3, 'valid')#第一个参数为深度
        self.bn1 = layers.BatchNormalization()

        self.conv2 = layers.Conv2DTranspose(128, 5, 2, 'valid')
        self.bn2 = layers.BatchNormalization()

        self.conv3 = layers.Conv2DTranspose(3, 4, 3, 'valid')#最后的深度降到RGB的3
...
def gradient_penalty(discriminator, batch_x, fake_image):
    ...
    interplate = t * batch_x + (1 - t) * fake_image#得到差值项，是tensor类型

    with tf.GradientTape() as tape:
        tape.watch([interplate])#必须设定成watch()才可以监视
...
def d_loss_fn(generator, discriminator, batch_z, batch_x, is_training):
    ...
    d_loss_real = celoss_ones(d_real_logits)#与标签1的交叉熵，最后返回的不是一个张量而是一个数，最后函数内部做了reduce_mean，
    d_loss_fake = celoss_zeros(d_fake_logits)
    gp = gradient_penalty(discriminator, batch_x, fake_image)

    loss = d_loss_fake + d_loss_real + 10. * gp#超参数这里默认升为1
...
def main():
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            ...
            z = tf.random.uniform([100, z_dim])#100个100维的向量
            fake_image = generator(z, training=False)
            img_path = os.path.join('/kaggle/working/images', 'wgan-%d.png'%epoch)
